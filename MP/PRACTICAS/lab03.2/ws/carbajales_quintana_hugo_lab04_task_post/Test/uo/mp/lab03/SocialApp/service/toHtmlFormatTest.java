...
class toHtmlFormatTest {
	...
	public void toHtmlText() {
		...
		assertEquals(result,socialNetwork.toHtmlFormat());
	}
	@Test
	public void toHtmlImage() {
		socialNetwork.addPost(image);
		ArrayList<Item>itemsResult=socialNetwork.getPosts();
		assertEquals(1,itemsResult.size());
		assertTrue(itemsResult.contains(image));
		String result="<img src= Marbella.jpg> caption </img>";
		assertEquals(result,socialNetwork.toHtmlFormat());
		
	}
	@Test
	public void photoAndMessageToHtml() {
		socialNetwork.addPost(image);
		socialNetwork.addPost(textMessage);
		ArrayList<Item>itemsResult=socialNetwork.getPosts();
		...
		assertTrue(itemsResult.contains(textMessage));
		String result="<img src= Marbella.jpg> caption </img>";
		String result2="<p>Hello!</p>";
		assertEquals(result+","+result2,socialNetwork.toHtmlFormat());
