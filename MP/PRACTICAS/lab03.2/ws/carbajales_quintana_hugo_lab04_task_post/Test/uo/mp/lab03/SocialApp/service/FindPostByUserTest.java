...
class FindPostByUserTest {
	...
	@Test
	public void userHasTwoPost() {
		socialNetwork.addPost(textMessage);
		Image image2 = new Image(user,fileName, "Here!");
		...
		ArrayList<Item>postUser=new ArrayList<Item>();
		postUser.add(textMessage);
		postUser.add(image2);
		assertTrue(postUser.contains(textMessage));
		assertTrue(postUser.contains(image2));
		assertEquals(2,postUser.size());
		
		assertEquals(postUser,socialNetwork.findPostByUser(user));
	}
	@Test
	public void userNoHasPost() {
		socialNetwork.addPost(image);
		Image image2 = new Image(user,fileName, "Here!");
		socialNetwork.addPost(image2);
		socialNetwork.addPost(new Image("Pepe",fileName, caption));
		...
		ArrayList<Item>postUser=new ArrayList<Item>();
		assertEquals(postUser,socialNetwork.findPostByUser("Pedro"));
	}
