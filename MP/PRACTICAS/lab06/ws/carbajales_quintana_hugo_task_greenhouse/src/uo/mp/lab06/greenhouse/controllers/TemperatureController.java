...
public class TemperatureController {
	 ...
	 * 
	 */
	private List<String> warmUp(double temp) { //Hacer en casa
		List<String>messages=new ArrayList<String>();
		messages.add(String.format("Current temp is %1.2f. It is too coold\n", temp));
		double diference=MIN_TEMPERATURE-temp;
		int doorsToClose=(int) (0.10*diference*doors.size())+1;
		messages.add(String.format("%d door must be close\n", doorsToClose));
	...
	private List<String> closeDoors(int doorsToClose) {
		...
		if(doorsToClose>0) {
			message.add(String.format("WARNING: No enought doors could been closed. "
					+ "%d more doors should be closed" , doorsToClose));
		}
		return message;
	}
	...
	private List<String> coolDown(double temp) {
		...
		double diference=temp-MAX_TEMPERATURE;
		int doorsToOpen=(int) (0.10*diference*doors.size())+1;
		messages.add(String.format("%d door must be opened\n", doorsToOpen));
	...
	private List<String> openDoors(int doorsToOpen) {
		...
		if(doorsToOpen>0) {
			message.add(String.format("WARNING: No enought doors could been opened. "
					+ "%d more doors should be opened" , doorsToOpen));
		}
		return message;
	}
